...
async function handleRequest(request) {
  ...
  try {
    ...
    const ga4Response = await fetch(`${GA4_ENDPOINT}?measurement_id=${GA4_MEASUREMENT_ID}&api_secret=${GA4_API_SECRET}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
function processEventData(data, request) {
  ...
  const origin = request.headers.get('Origin') || '';
  const host = origin ? new URL(origin).host : (referer ? new URL(referer).host : request.headers.get('Host'));
  ...
  switch (processedData.name) {
    case 'page_view':
      ...
      if (!processedData.params.page_path && processedData.params.page_location) {
        try {
    ...
    case 'add_to_cart':
      ...
      if (processedData.params.items && Array.isArray(processedData.params.items)) {
        ...
        if (!processedData.params.value && processedData.params.items.length > 0) {
    ...
    case 'view_item':
      ...
      if (processedData.params.items && Array.isArray(processedData.params.items)) {
        ...
        if (!processedData.params.value && processedData.params.items.length > 0) {
          processedData.params.value = processedData.params.items.reduce(
            (total, item) => total + (parseFloat(item.price) || 0), 0
          );
        }
    ...
    case 'view_item_list':
      ...
      // Ensure items array exists and has required fields
      if (processedData.params.items && Array.isArray(processedData.params.items)) {
        processedData.params.items = processedData.params.items.map((item, index) => {
          // Ensure required item fields
          if (!item.item_id) {
