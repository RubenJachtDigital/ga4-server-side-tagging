...
jQuery(document).ready(function($) {
    ...
    $('#ga4_ab_tests_enabled').change(function() {
        if ($(this).is(':checked')) {
    ...
    $('#ga4_click_tracks_enabled').change(function() {
        if ($(this).is(':checked')) {
    ...
    // Add new A/B test
    $('#add_ab_test').click(function() {
        var container = $('#ab_tests_container');
        var index = container.children().length;
        
        var html = '<div class="ab-test-item" data-index="' + index + '">' +
            '<table class="form-table">' +
            '<tr><th scope="row"><label>Test Name</label></th>' +
            '<td><input type="text" name="ab_test_name[]" placeholder="e.g., Button Color Test" class="regular-text" /></td></tr>' +
            '<tr><th scope="row"><label>Variant A CSS Class</label></th>' +
            '<td><input type="text" name="ab_test_class_a[]" placeholder="e.g., .button-red" class="regular-text" /></td></tr>' +
            '<tr><th scope="row"><label>Variant B CSS Class</label></th>' +
            '<td><input type="text" name="ab_test_class_b[]" placeholder="e.g., .button-blue" class="regular-text" /></td></tr>' +
            '<tr><th scope="row"><label>Enabled</label></th>' +
            '<td><input type="checkbox" name="ab_test_enabled[]" value="1" checked /> <span>Test is active</span></td></tr>' +
            '</table>' +
            '<button type="button" class="button remove-ab-test">Remove Test</button><hr>' +
            '</div>';
    ...
    // Add new click track
    $('#add_click_track').click(function() {
        var container = $('#click_tracks_container');
        var index = container.children().length;
        
        var html = '<div class="click-track-item" data-index="' + index + '">' +
            '<table class="form-table">' +
            '<tr><th scope="row"><label>Event Name</label></th>' +
            '<td><input type="text" name="click_track_name[]" placeholder="e.g., download_pdf, cta_click" class="regular-text" />' +
            '<p class="description">This becomes the GA4 event name (automatically sanitized)</p></td></tr>' +
            '<tr><th scope="row"><label>CSS Selector</label></th>' +
            '<td><input type="text" name="click_track_selector[]" placeholder="e.g., .download-btn, #cta-button" class="regular-text" />' +
            '<p class="description">CSS selector for elements to track</p></td></tr>' +
            '<tr><th scope="row"><label>Enabled</label></th>' +
            '<td><input type="checkbox" name="click_track_enabled[]" value="1" checked /> <span>Track is active</span></td></tr>' +
            '</table>' +
            '<button type="button" class="button remove-click-track">Remove Track</button><hr>' +
            '</div>';
    ...
    // Serialize A/B tests before form submit
    $('form').on('submit', function() {
        var abTests = [];
        $('.ab-test-item').each(function() {
            var $item = $(this);
            abTests.push({
                name: $item.find('input[name="ab_test_name[]"]').val(),
                class_a: $item.find('input[name="ab_test_class_a[]"]').val(),
                class_b: $item.find('input[name="ab_test_class_b[]"]').val(),
                enabled: $item.find('input[name="ab_test_enabled[]"]').is(':checked')
            });
        });
        $('#ga4_ab_tests_config').val(JSON.stringify(abTests));

        // Serialize click tracks
        var clickTracks = [];
        $('.click-track-item').each(function() {
            var $item = $(this);
            clickTracks.push({
                name: $item.find('input[name="click_track_name[]"]').val(),
                selector: $item.find('input[name="click_track_selector[]"]').val(),
                enabled: $item.find('input[name="click_track_enabled[]"]').is(':checked')
            });
        });
        $('#ga4_click_tracks_config').val(JSON.stringify(clickTracks));
    });
});
</script>
