...
class GA4_Server_Side_Tagging_Endpoint
{
    ...
    public function register_routes()
    {
        ...
        // Bot validation endpoint for Simple requests
        register_rest_route('ga4-server-side-tagging/v1', '/validate-bot', array(
            'methods' => 'GET',
            'callback' => array($this, 'validate_bot'),
            'permission_callback' => array($this, 'check_bot_validation_permission'),
        ));
    }
    ...
    /**
     * Permission check for bot validation endpoint.
     *
     * @since    1.0.0
     * @param    \WP_REST_Request    $request    The request object.
     * @return   bool                           Whether the request has permission.
     */
    public function check_bot_validation_permission($request)
    {
        $client_ip = $this->get_client_ip($request);
        
        // Rate limiting for bot validation (same as send-event)
        $rate_limit_key = 'ga4_bot_validation_rate_limit_' . $client_ip;
        $rate_limit_count = get_transient($rate_limit_key);
        
        if ($rate_limit_count && $rate_limit_count >= 100) {
            $this->log_security_failure($request, 'RATE_LIMIT_EXCEEDED', 'Bot validation rate limit exceeded: ' . $rate_limit_count . ' requests from IP: ' . $client_ip);
            return false;
        }
        
        // Update rate limit counter
        $new_count = $rate_limit_count ? $rate_limit_count + 1 : 1;
        set_transient($rate_limit_key, $new_count, 3600); // 1 hour window
        
        // Origin validation
        if (!$this->validate_request_origin($request)) {
            $this->log_security_failure($request, 'ORIGIN_VALIDATION_FAILED', 'Bot validation origin check failed');
            return false;
        }
     ...
     * @return   \WP_REST_Response              The response object.
     */
    public function validate_bot($request)
    {
        try {
            $client_ip = $this->get_client_ip($request);
            $user_agent = $request->get_header('user-agent');
            ...
            // Store validation result in session for performance
            if (!session_id()) {
                session_start();
            }
            $_SESSION['ga4_bot_validation_passed'] = true;
            $_SESSION['ga4_bot_validation_time'] = time();
            $_SESSION['ga4_bot_validation_ip'] = $client_ip;
            $_SESSION['ga4_bot_validation_headers'] = [
                'user_agent' => $user_agent,
                'accept_language' => $accept_language,
                'accept_encoding' => $accept_encoding,
                'accept' => $accept,
            ...
            $this->logger->error('Bot validation error: ' . $e->getMessage());
